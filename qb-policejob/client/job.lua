...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) > 7.0 and not IsPedInAnyVehicle(ped) then
            QBCore.Functions.Notify("There is no car to impound around you")
            else if #(pos - vehpos) < 3.0 and not IsPedInAnyVehicle(ped) then
                local plate = QBCore.Functions.GetPlate(vehicle)
                TriggerServerEvent("police:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
                QBCore.Functions.Progressbar('impound', 'Impounding Car', 1000, false, true, {
                ...
                QBCore.Functions.Notify("Vehicle Impounded")
            else
                QBCore.Functions.Notify("Try to move closer the the car")
            end
        end
    end
end)
