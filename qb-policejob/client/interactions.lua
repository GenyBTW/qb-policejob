...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff, AttachEntity)
    if not isHandcuffed then
        ...
        if success then
            ...
            QBCore.Functions.Notify("You broke out now run mf!!!")
            isHandcuffed = false
            isEscorted = false
            TriggerServerEvent("police:server:SetHandcuffStatus", false)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "Uncuff", 0.2)
            TriggerServerEvent("ServerEmoteCancel")
        else
            QBCore.Functions.Notify("You failed to break out bozo")
            isHandcuffed = true
            TriggerServerEvent("police:server:SetHandcuffStatus", true)
            local boneoffsets = {
                ["p_cs_cuffs_02_s"] = {
                    bone = 18905,
                    offset = vector3(0.0, 0.07, 0.06),
                    rotation = vector3(110.0, 30.0, 60.0),
                },
            }
            ...
            AttachEntity(PlayerPedId(), "p_cs_cuffs_02_s")
            ClearPedTasksImmediately(ped)
